...
void* vfs_init(struct fuse_conn_info *conn, struct fuse_config *cfg) {
    ...
    string backend_root = vfs_backend_path("/") ;
...
int vfs_unlink(const char *path) {
    cout << "[VFS] unlink called for: " << path << endl;
    string backend_path = vfs_backend_path(path);
    if (unlink(backend_path.c_str()) == -1) {
        cerr << "[VFS] ERROR: Failed to delete file: " << backend_path << endl;
        return -errno;
    }
    cout << "[VFS] File deleted from backend: " << backend_path << endl;
    VersionManager::delete_all_versions(backend_path);
    return 0;
}
